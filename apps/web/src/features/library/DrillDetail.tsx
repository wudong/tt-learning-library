...
export function DrillDetail({ nodeId, navigate }: { nodeId: string; navigate: (to: string) => void }) {
  ...
  const linkedVideoIds = new Set(resources.data?.videos.map((video) => video.id) ?? [])
  ...
  return (
    <section className="library-detail-page">
      ...
      {resources.data?.drill && (() => {
        const drill = resources.data.drill
        return <>
          <header className="library-detail-hero">
            ...
            <div className="row">
              ...
              <button className="button secondary detail-pin" disabled={pin.isPending} onClick={() => pin.mutate(!resources.data.isPinned)}>
                {resources.data.isPinned ? <><PinOff size={16} /> Unpin</> : <><Pin size={16} /> Pin to top</>}
              </button>
            </div>
          </header>
          ...
          <div className="library-detail-grid">
            ...
            {resources.data.drillSteps.length > 0 && <article className="card">
              <h2>Stroke sequence and spin</h2>
              <ol className="drill-steps">{resources.data.drillSteps.map((step) => <li key={step.id}>
            ...
            <article className="card">
              ...
              {resources.data.skills.length > 0 ? <div className="detail-link-list">{resources.data.skills.map((skill) =>
            ...
            <article className="card">
              ...
              {resources.data.videos.length > 0 ? <div className="resource-videos">{resources.data.videos.map((video) =>
