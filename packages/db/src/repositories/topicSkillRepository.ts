...
export class TopicSkillRepository {
  ...
  async createSkill(input: { userId: string; nodeId: string; name: string; description?: string | null; topicId?: string | null; difficulty?: string | null; status?: string; isSystem?: boolean }) {
    const now = nowIso(); const row = { id: createId('skill'), node_id: input.nodeId, user_id: input.userId, topic_id: input.topicId ?? null, name: input.name, description: input.description ?? null, difficulty: input.difficulty ?? null, status: input.status ?? 'not_started', is_system: input.isSystem ? 1 : 0, is_pinned: 0, created_at: now, updated_at: now, deleted_at: null }
